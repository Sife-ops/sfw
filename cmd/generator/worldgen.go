...
func Worldgen() {
	...
	cmdRmRfWorld := exec.CommandContext(ctx, "rm", "-rf",
		fmt.Sprintf("%s/tmp/mc/data/world", lib.MustString(os.Getwd())),
	)
	...
	// nether
	forceloadedNetherChunks := []lib.Coords{}
	for _, v := range job.NetherChunksToBastion() {
		forceloadedNetherChunks = append(forceloadedNetherChunks, lib.Coords{X: v.X, Z: v.Z})
		if ec, err := McExec(ctx, mc.ID, []string{"rcon-cli",
			fmt.Sprintf(
	...
	OpenRegion:
		region, err := region.Open(fmt.Sprintf(
			...
			lib.MustString(os.Getwd()), regionX, regionZ,
		))
		if err != nil {
		...
		for xC := x1 / 16; xC < (x2+1)/16; xC++ {
			for zC := z1 / 16; zC < (z2+1)/16; zC++ {
				...
				data, err := region.ReadSector(lib.ToSector(xC), lib.ToSector(zC))
				if err != nil {
					log.Printf("error %v", err)
					WorldgenRecovering <- job
					return
				...
				if obby >= 30 && magma >= 10 && lava >= 30 {
					magmaRavineChunks = append(magmaRavineChunks, lib.Coords{X: xC, Z: zC})
				}
			}
		}
	...
	region, err := region.Open(fmt.Sprintf(
		...
		lib.MustString(os.Getwd()), netherChunkCoords[0].X, netherChunkCoords[0].Z,
	))
	if err != nil {
	...
	for _, v := range netherChunkCoords {
		...
		data, err := region.ReadSector(lib.ToSector(v.X), lib.ToSector(v.Z))
		if err != nil {
			log.Printf("error %v", err)
			WorldgenRecovering <- job
			return
